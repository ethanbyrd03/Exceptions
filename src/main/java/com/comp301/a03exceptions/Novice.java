...
public class Novice {
  ...
  public static int section1(int n) {
    if (n < 0) {
      throw new UnsupportedOperationException("Required operation is not supported.");
    } else if (n == 0) {
      throw new IndexOutOfBoundsException("index is invalid.");
    } else if (n == 1) {
      throw new ArrayIndexOutOfBoundsException("Array index is invalid.");
    } else if (n == 2) {
      throw new IllegalStateException("Invalid state was detected in the application.");
    } else if (n == 3) {
      throw new IllegalArgumentException("Invalid argument value was provided to the method.");
    } else if (n == 4) {
      throw new ArithmeticException("Arithmetic error occurred during execution of the method.");
    } else if (n % 5 == 0) {
